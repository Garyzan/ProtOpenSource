...
void getMicrophoneLevel()
{
    ...
    int read_val = analogRead(MIC_PIN);
    int microphoneLevel = rollingAverage(abs(read_val - baseline)); // Read pin value
    Serial.print(String(read_val) + " " + String(microphoneLevel) + "\n");
